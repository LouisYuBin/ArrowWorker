...
class MongoDb
{
    ...
    public function update($table, $filter, $update)
    {
        ...
        try {
            $result = self::$mongoManager->executeBulkWrite(self::$mongoConfig['dbName'] .
                '.' .
                $table, $bulkWrite);
            ...
            if ($writeConcernError = $result->getWriteConcernError()) {
                $updateRlt = [
                    'result' => $writeConcernError->getCode(),
                    'msg'    => $writeConcernError->getMessage(),
                ];
            }
            // Check if any write operations did not complete at all
            foreach ($result->getWriteErrors() as $writeError) {
                $updateRlt = [
                    'result' => $writeError->getCode(),
